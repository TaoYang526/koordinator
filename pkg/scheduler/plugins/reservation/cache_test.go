...
func TestReservationCache_PreAllocatableOperations(t *testing.T) {
	...
	t.Run("add and retrieve candidates", func(t *testing.T) {
		...
		node1Pod1 := createTestPodWithScore("node1-pod1", "node1", "100")
		node1Pod2 := createTestPodWithScore("node1-pod2", "node1", "50")
		node1Pod3 := createTestPodWithScore("node1-pod3", "node1", "200")
		node2Pod1 := createTestPodWithScore("node2-pod1", "node2", "150")
	...
	t.Run("update score and re-sort", func(t *testing.T) {
		...
		updatePod := createTestPodWithScore("update-pod", "node1", "30")
	...
	t.Run("edge cases", func(t *testing.T) {
		...
		nonExistPod := createTestPodWithScore("non-exist", "node1", "100")
...
// TODO remove this
func createTestPodWithScore(uid, nodeName, score string) *corev1.Pod {
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			UID: types.UID(uid),
