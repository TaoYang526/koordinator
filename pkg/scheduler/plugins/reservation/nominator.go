...
type nominator struct {
	...
	// nominatedPreAllocatables is map keyed by a Reservation UID to the node name where there are multiple pre-allocatable
	// pods being nominated.
	nominatedPreAllocatables map[types.UID]map[string][]*corev1.Pod
...
func (nm *nominator) deleteReservePod(pod *corev1.Pod) {
	...
	delete(nm.nominatedPreAllocatables, pod.UID)
...
func (nm *nominator) RemoveNominatedPreAllocation(pod *corev1.Pod) {
	nm.lock.Lock()
	defer nm.lock.Unlock()
	// Clean both single and multiple pre-allocation nominations
	delete(nm.nominatedPreAllocatable, pod.UID)
	delete(nm.nominatedPreAllocatables, pod.UID)
}
...
func (nm *nominator) AddNominatedPreAllocations(rInfo *frameworkext.ReservationInfo, nodeName string, pods []*corev1.Pod) {
	...
	nodeToPreAllocatables := nm.nominatedPreAllocatables[rInfo.UID()]
	if nodeToPreAllocatables == nil {
		nodeToPreAllocatables = map[string][]*corev1.Pod{}
		nm.nominatedPreAllocatables[rInfo.UID()] = nodeToPreAllocatables
	}
...
func (nm *nominator) GetNominatedPreAllocations(rInfo *frameworkext.ReservationInfo, nodeName string) []*corev1.Pod {
	nm.lock.RLock()
	defer nm.lock.RUnlock()
	return nm.nominatedPreAllocatables[rInfo.UID()][nodeName]
}
